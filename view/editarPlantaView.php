    ...
    <div class="card shadow p-4">
        ...
        <?php if ($status === 'con_especimenes'): ?>
            <div class="alert alert-danger">
                <strong>Acción bloqueada.</strong> No se puede inhabilitar esta planta porque tiene especímenes vinculados activos.
            </div>
        <?php elseif ($status === 'invalido'): ?>
            <div class="alert alert-warning">El nombre de la planta es obligatorio.</div>
        <?php elseif ($status === 'error'): ?>
            <div class="alert alert-danger">Error al actualizar el registro. El nombre podría estar duplicado.</div>
        <?php endif; ?>
        ...
        <form method="POST" action="?controlador=Planta&accion=editar" onsubmit="return confirm('¿Desea modificar esta planta hospedadora?');">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nombre" class="form-control form-control-lg"
                       value="<?php echo htmlspecialchars($planta['nombre']); ?>" required>
                <div class="form-text">Nombre común o científico de la planta hospedadora.</div>
            </div>
            ...
            <div class="d-flex justify-content-between mt-4">
                ...
                <a href="?controlador=Planta&accion=inhabilitar&id=<?php echo htmlspecialchars($planta['id_planta']); ?>"
                   class="btn btn-danger"
                   onclick="return confirm('¿Seguro que desea inhabilitar esta planta? Solo es posible si no tiene especímenes vinculados.');">
                    Inhabilitar Planta
                </a>
            </div>
        </form>
